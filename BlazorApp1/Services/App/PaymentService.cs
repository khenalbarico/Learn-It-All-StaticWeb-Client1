...
public class PaymentService(IApiClient _api) : IPaymentService
{
    ...
    public Task<PaymentStatusResult> GetPaymentStatus(string paymentIntentId)
        => _api.GetAsync<PaymentStatusResult>(ApiFunctions.GetPaymentStatus, new { PaymentIntentId = paymentIntentId });
}
